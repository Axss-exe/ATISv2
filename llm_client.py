...
DETERMINISM NOTES:
- Default temperature is now 0.0 for maximum determinism.
- The client supports an optional seed parameter for reproducible outputs.
- All LLM calls in the pipeline should use temperature=0.0.
"""
...
def load_config() -> LLMConfig:
    ...
    logger.info("LLM provider=%s model=%s fallback=%s temperature=%s",
                config.provider, config.model, config.fallback_model or "none", config.temperature)
    return config
...
class LLMClient:
    ...
    def chat(self, messages: List[Dict[str, str]], temperature: float | None = None,
        ...
        Args:
            ...
            temperature: Sampling temperature. Defaults to config.temperature (0.0 for determinism).
            max_tokens: Maximum tokens to generate.
            seed: Optional seed for reproducible outputs (provider-dependent).
        """
        ...
        for model in models:
            for attempt in range(1, self.config.max_retries + 1):
                try:
                    return self._request(model, messages, temperature, max_tokens, seed)
    ...
    def _request(self, model: str, messages: List[Dict[str, str]], temperature: float | None,
                 max_tokens: int | None, seed: int | None) -> str:
        payload = {
            "model": model,
            "messages": messages,
        ...
        # Add seed if provided (for providers that support it)
        if seed is not None:
            payload["seed"] = seed
        req = request.Request(
...
def chat(messages: List[Dict[str, str]], temperature: float | None = None,
         max_tokens: int | None = None, seed: int | None = None) -> str:
    return get_client().chat(messages, temperature=temperature, max_tokens=max_tokens, seed=seed)
